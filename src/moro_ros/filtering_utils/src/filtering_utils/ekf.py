...
class EKF:
    def __init__(self, state_vector_size, control_size, measurement_size):
        ...
        self.cov_matrix = 1.0 * np.identity(state_vector_size)
        self.state = np.zeros((control_size, control_size))
        ...
        #self.obs_j_state = np.zeros((state_vector_size, state_vector_size))
    ...
    def update(self, lmark, m_x, m_y, a, s):
        # H
        ...
        hx = self.hx(self.state_vector[0], self.state_vector[1], self.state_vector[2], lmark[0], lmark[1], s)
        ...
        """
        K_z = np.dot(K, y)
        K_h = np.dot(K, self.obs_j_state)
        return K_z, K_h
        pass

    def hx(self, x, y, theta, m_x, m_y, s):
        dist = np.sqrt((m_x-x)**2+(m_y-y)**2)
        Hx = np.array([dist, np.arctan2(m_y-y, m_x-x)-theta])
        return Hx

    def propagate_state(self, x, y, theta, v, w):
        
        self.state_vect = self.state_vector[0] + (-v/w)*np.sin(theta)+(v/w)*np.sin(theta+w*self.t)
        self.state_vector[1] = self.state_vector[1] + (v/w)*np.cos(theta)-(v/w)*np.cos(theta+w*self.t)
        self.state_vector[2] = self.state_vector[2] + w*self.t
    ...
    def motion_jacobian_state_vector(self, x, y, theta, v, w):
        ...
        self.motion_j_state[1,:] = np.array(
            [0, 1, -(v/w)*np.sin(theta)+(v/w)*np.sin(theta+w*self.t)])
    ...
    def observation_jacobian_state_vector(self, x, y, theta, m_x, m_y):
        ...
        self.obs_j_state[1,:] = np.array(
            [-(m_y-y)/((m_x-x)**2+(m_y-y)**2), (m_x-x)/((m_x-x)**2+(m_y-y)**2), -1])
        #self.obs_j_state[2,:] = np.array([0, 0, 0])
